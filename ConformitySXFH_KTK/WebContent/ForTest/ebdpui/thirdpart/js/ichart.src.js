...
(function($) {
	...
	Cans.prototype = {
		...
		sector3D : function() {
			var x0, y0,sPaint = function(x, y, a, b, s, e, ccw, h, c) {
				...
				angle = s;
				this.fillStyle(iChart.dark(c)).moveTo(x + a * cos(s), y + (b * sin(s))).beginPath();
				while (angle <= e) {
					Lo.call(this, angle);
					angle = angle + inc;
				}
				...
				var q1 = iChart.quadrantd(s),q2 = iChart.quadrantd(e);
				c = iChart.dark(c);
				if (q1==1||q1==2)
		...
		cube : function(x, y, xv, yv, width, height, zdeep, bg, b, bw, bc, sw) {
			 ...
			 */
			this.polygon(iChart.dark(bg), b, bw, bc, false, false, [{x:x, y:y}, {x:x1, y:y1}, {x:x1 + width, y:y1}, {x:x + width, y:y}]);
			this.polygon(bg, b, bw, bc, false, false, [{x:x, y:y}, {x:x, y:y + height}, {x:x + width, y:y + height}, {x:x + width,y: y}]);
			this.polygon(iChart.dark(bg), b, bw, bc, false, false, [{x:x + width, y:y}, {x:x1 + width, y:y1}, {x:x1 + width, y:y1 + height}, {x:x + width, y:y + height}]);
			return this;
		},
		cube3D : function(x, y, rotatex, rotatey, angle, w, h, zh, b, bw, bc, styles) {
			...
			if (angle) {
				var P = iChart.vectorP2P(rotatex, rotatey);
		...
		box : function(x, y, w, h, b, bg, shadow, m,last) {
			...
			if (b.enable) {
				var j = b.width, c = b.color, r = b.radius, f = iChart.isNumber(j);
				j = iChart.parsePadding(j);
				...
				y -= m*(j[0] / 2);
				j = f ? j[0] : j;
				r = (!f ||!r|| r == 0 || r == '0') ? 0 : iChart.parsePadding(r);
			}
		...
		addEvent : function(type, fn, useCapture) {
			iChart.Event.addEvent(this.canvas, type, fn, useCapture);
		}
	}
	...
	iChart.Chart = iChart.extend(iChart.Painter, {
		...
		configure : function() {
			 ...
			 * invoked the super class's configuration
			 */
			iChart.Chart.superclass.configure.apply(this, arguments);
		...
		animation : function(_) {
			...
			if (_.variable.animation.time < _.duration) {
				...
				iChart.requestAnimFrame(function() {
					_.animation(_);
				});
			} else {
				iChart.requestAnimFrame(function() {
		...
		doSort:function(){
			this.components.sor(function(p, q){
				return (iChart.isArray(p)?(p.zIndex||0):p.get('z_index'))>(iChart.isArray(q)?(q.zIndex||0):q.get('z_index'))});
		},
		commonDraw : function(_,e) {
			
			if (!_.redraw) {
				iChart.Assert.isTrue(_.Rendered, _.type + ' has not rendered');
				iChart.Assert.isTrue(_.data&&_.data.length>0,_.type + '\'s data is empty');
				iChart.Assert.isTrue(_.initialization, _.type + ' Failed to initialize');
				_.doSort();
		...
		create : function(_,shell) {
			...
			if(_.get('fit')){
				var w = window.innerWidth,
			    	...
			    	style = iChart.getDoc().body.style;
			    style.padding = "0px";
			    style.margin = "0px";
		...
		initialize : function() {
			...
			if(_.Rendered && !_.initialization){
				if(d&&d.length>0){
					parse.call(_,d,_);
				}else if(iChart.isString(_.get('url'))){
		 ...
		 */
		oneWay:function(_){
			var E = _.variable.event,comb=_.Combination,tot=!_.get('turn_off_touchmove')&&!comb, mCSS = !iChart.touch&&_.get('default_mouseover_css')&&!comb, O, AO,events = iChart.touch?['touchstart','touchmove']:['click','mousemove'];
			_.stopEvent = false;
			...
			if(!comb){
				events.each(function(it) {
					_.T.addEvent(it, function(e) {
						...
						_.fireEvent(_, it, [_, iChart.Event.fix(e)]);
					}, false);
				});
			}
		...
		doConfig : function() {
			...
			iChart.Chart.superclass.doConfig.call(this);
			...
			if(iChart.isFunction(_.get('doAnimation'))){
				_.doAnimation = _.get('doAnimation');
			}
			_.animationArithmetic = iChart.getAA(_.get('animation_timing_function'));
			/**
			 * destroy exist dom
			 */
			_.destroy();
			...
			if(!_.Combination){
				...
				_.duration = ceil(_.get('animation_duration') * iChart.FRAME / 1000);
				...
				*/
				...
				if (iChart.isString(_.get('title'))) {
					_.push('title', iChart.applyIf({
						text : _.get('title')
					}, _.default_.title));
				}
				if (iChart.isString(_.get('subtitle'))) {
					_.push('subtitle', iChart.applyIf({
						text : _.get('subtitle')
					}, _.default_.subtitle));
				}
				if (iChart.isString(_.get('footnote'))) {
					_.push('footnote', iChart.applyIf({
						text : _.get('footnote')
					}, _.default_.footnote));
				}
				...
				if (_.get('title.text') != '') {
					...
					_.title = new iChart.Text(_.get('title'), _);
					...
					if (st) {
						...
						_.subtitle = new iChart.Text(_.get('subtitle'), _);
				...
				if (_.get('footnote.text') != '') {
					...
					_.footnote = new iChart.Text(_.get('footnote'), _);
			...
			if (_.get('legend.enable')) {
				_.legend = new iChart.Legend(iChart.apply({
...
iChart.Donut2D = iChart.extend(iChart.Pie, {
	...
	doConfig : function() {
		...
		if (iChart.isString(_.get('center'))) {
			_.push('center', iChart.applyIf({
				text : _.get('center')
			}, _.default_.center));
		}
		...
		if (_.get('center.text') != '') {
			...
			_.center = new iChart.Text(_.get('center'), _);
