// 以下代码引用，稍微修改了点
var placeholderTools = (function($) {
    var isInputSupported = 'placeholder' in document.createElement('input');
    var isTextareaSupported = 'placeholder' in document.createElement('textarea');
	//某些Android手机有问题，默认都不支持
	var prototype = {};
	...
	if (isInputSupported && isTextareaSupported) {
		...
		placeholder = prototype.placeholder = function() {
			var $this = this;
			var $temp=$this.filter((isInputSupported ? 'textarea' : ':input') + '[placeholder]').unbind({
			...
			jQuery.each($temp,function(index,element){
		...
		$(function() {
			...
			$(document).delegate('form', 'submit.placeholder', function() {
				...
				var $inputs = $('.placeholder', this).each(clearPlaceholderForFormSubmit);
				setTimeout(function() {
	...
	function clearPlaceholderForFormSubmit(event, value) {
		var input = this, $input = $(input);
		// 修改演示四出现的问题
		if ((input.value == $input.attr('placeholder') || input.value == '') && $input.hasClass('placeholder')) {
			if ($input.data('placeholder-password')) {
